    ...
    <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/88 shadow-[0_10px_24px_rgba(15,23,42,0.04)] backdrop-blur">
      <div className="mx-auto flex w-full max-w-[1240px] flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6">
        ...
        <nav className="flex flex-1 flex-wrap items-center justify-end gap-2 text-xs sm:text-sm">
          <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200/90 bg-slate-50/85 p-1">
            {links.map(([to, label]) => (
              <NavLink
                key={to}
                to={to}
          ...
          <NavLink
            to="/profile"
            className={({ isActive }) =>
              `ml-2 hidden min-w-[260px] items-center gap-3 rounded-xl border px-3 py-2 text-left transition lg:flex ${
                isActive
          ...
          <button
            ...
            disabled={signingOut}
            className="rounded-lg border border-slate-300 bg-white px-3 py-1.5 font-medium text-slate-700 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60"
          >
            Sign Out All
          </button>
          ...
          <button
            ...
            disabled={signingOut}
            className="rounded-lg border border-blue-700 bg-blue-700 px-3 py-1.5 font-medium text-white hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-60"
          >
