...
function fallbackRecipient(payload) {
  return payload?.recipientEmail || payload?.organizerEmail || payload?.participantEmail || env.defaultRecipientEmail || null;
}
...
export function buildTopicNotification(topic, payload) {
  switch (topic) {
    ...
    case env.resourceAllocationTopic:
      return {
        recipient: fallbackRecipient(payload),
        subject: `Resource allocation update for event ${payload.eventId || ""}`.trim(),
